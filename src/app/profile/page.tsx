      ...
      <div className="bg-congress-blue-900 rounded-[calc(2rem+1rem)] p-4">
        <div className="space-y-4 w-full bg-background rounded-4xl px-6 py-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h1 className="text-xl font-semibold text-congress-blue-900">Profile</h1>
              <div className="mt-1">
                {currentUser.isLoading ? (
                  <ProfileNameSkeleton />
                ) : currentUser.isError ? (
                  <InlineBanner tone="error" title="Could not load user" detail="Please try again." />
                ) : currentUser.data ? (
                  <p className="text-sm text-congress-blue-900/70">
                    {currentUser.data.name} {currentUser.data.last_name}
                  </p>
                ) : (
                  <InlineBanner tone="info" title="Not logged in" detail="Log in to view your profile." />
                )}
              </div>
            </div>
          </div>
